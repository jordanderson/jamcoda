...
router.get('/by-date', route('get files', async (req, res) => {
...
router.get('/:id', route('get file detail', async (req, res) => {
...
router.put('/:id/completion', route('update file completion', async (req, res) => {
  const id = parseId(req.params.id);
  if (!id) {
    return res.status(400).json({ error: 'Invalid file id' });
  }
...
router.get('/:id/download', route('download file', async (req, res) => {
  const id = parseId(req.params.id);
  if (!id) {
    return res.status(400).json({ error: 'Invalid file id' });
  }
