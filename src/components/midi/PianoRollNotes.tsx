 ...
 * `currentTime` on every animation frame, so without this boundary React
 * would reconcile every rect ~60 times a second and playback would stutter.
 *
 * `rects` only changes when the loaded sequence does, so this subtree is
 * skipped entirely during playback. Magenta's visualizer avoided the problem
 * by drawing into the SVG imperatively, outside React's tree. This keeps the
